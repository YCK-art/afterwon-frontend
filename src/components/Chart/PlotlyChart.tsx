'use client'

import { useEffect, useRef, useState } from 'react'
import { FiEdit2, FiDownload, FiMaximize2, FiSliders } from 'react-icons/fi'

declare global {
  interface Window {
    Plotly: any;
  }
}

interface PlotlyChartProps {
  data: any
  height?: number
  onTitleChange?: (newTitle: string) => void
  onChartExpand?: (chartData: any, title: string) => void
  showTitleEditor?: boolean
  showControls?: boolean
  messageId?: string
  onColorChange?: (messageId: string, colors: {[key: number]: string}) => void
  savedColors?: {[key: number]: string}
}

export default function PlotlyChart({ data, height = 520, onTitleChange, onChartExpand, showTitleEditor = true, showControls = true, messageId, onColorChange, savedColors }: PlotlyChartProps) {
  const chartRef = useRef<HTMLDivElement>(null)
  const [isEditing, setIsEditing] = useState(false)
  const [editTitle, setEditTitle] = useState('')
  const [currentTitle, setCurrentTitle] = useState('')
  const [showColorPicker, setShowColorPicker] = useState(false)
  const [traceColors, setTraceColors] = useState<{[key: number]: string}>({})
  const [activeColorPicker, setActiveColorPicker] = useState<number | null>(null)
  const colorPickerRef = useRef<HTMLDivElement>(null)
  const paletteRef = useRef<HTMLDivElement>(null)
  const [showTitleModal, setShowTitleModal] = useState(false)
  const [titleHovered, setTitleHovered] = useState(false)
  const titleModalRef = useRef<HTMLDivElement>(null)

  // HTML 태그를 제거하고 깔끔한 텍스트만 추출하는 함수
  const stripHtmlTags = (html: string): string => {
    if (!html) return ''

    // HTML 태그를 제거하고 실제 텍스트만 추출
    const tempDiv = document.createElement('div')
    tempDiv.innerHTML = html
    const cleanText = tempDiv.textContent || tempDiv.innerText || ''

    // 추가적으로 연속된 공백을 하나로 정리
    return cleanText.replace(/\s+/g, ' ').trim()
  }

  // 차트 제목 생성 로직 개선
  const generateBetterTitle = (plotData: any[], layoutData: any) => {
    if (layoutData?.title?.text) {
      // HTML 태그가 포함된 제목에서 깔끔한 텍스트만 추출
      return stripHtmlTags(layoutData.title.text)
    }

    if (!plotData || plotData.length === 0) return '데이터 시각화'

    const trace = plotData[0]
    const chartType = trace.type || 'scatter'
    
    // 차트 타입별 제목 생성
    switch (chartType) {
      case 'bar':
        if (trace.x && trace.y) {
          const xLabel = trace.name || (Array.isArray(trace.x) ? '카테고리' : 'X축')
          const yLabel = Array.isArray(trace.y) ? '수량' : 'Y축'
          return `${xLabel}별 ${yLabel} 분석`
        }
        return '막대 차트 분석'
      
      case 'pie':
        if (trace.labels) {
          return `${trace.name || '항목'}별 구성 비율`
        }
        return '구성 비율 분석'
      
      case 'line':
      case 'scatter':
        if (trace.x && trace.y) {
          const xLabel = trace.name || 'X값'
          const yLabel = Array.isArray(trace.y) ? 'Y값' : 'Y축'
          return `${xLabel}과 ${yLabel}의 상관관계`
        }
        return '추세 분석'
      
      case 'histogram':
        return `${trace.name || '데이터'} 분포 분석`
      
      case 'box':
        return `${trace.name || '데이터'} 박스 플롯 분석`

      case 'choropleth':
        if (trace.locations && trace.z) {
          return `${trace.name || '지역'}별 ${trace.colorbar?.title?.text || '수치'} 분포`
        }
        return '지역별 분포 지도'

      case 'scattergeo':
        return `${trace.name || '지역'} 지도 분석`

      default:
        if (trace.name) {
          return `${trace.name} 데이터 분석`
        }
        return '데이터 시각화 분석'
    }
  }

  // 제목 수정 기능
  const handleTitleEdit = () => {
    const cleanTitle = stripHtmlTags(currentTitle)
    setEditTitle(cleanTitle)
    setShowTitleModal(true)
  }

  const handleTitleSave = () => {
    const trimmedTitle = editTitle.trim()
    if (trimmedTitle && trimmedTitle !== stripHtmlTags(currentTitle)) {
      setCurrentTitle(trimmedTitle)
      if (onTitleChange) {
        onTitleChange(trimmedTitle)
      }
      // Plotly 차트 제목 업데이트
      if (window.Plotly && chartRef.current && data) {
        const newLayout = {
          ...data.layout,
          title: {
            ...data.layout?.title,
            text: trimmedTitle,
            font: { size: 16, color: '#1f2937' },
            x: 0.02,
            xanchor: 'left'
          }
        }
        window.Plotly.relayout(chartRef.current, newLayout)
      }
    }
    setShowTitleModal(false)
  }

  const handleTitleCancel = () => {
    const cleanTitle = stripHtmlTags(currentTitle)
    setEditTitle(cleanTitle)
    setShowTitleModal(false)
  }

  const handleKeyPress = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter') {
      e.preventDefault()
      handleTitleSave()
    } else if (e.key === 'Escape') {
      e.preventDefault()
      handleTitleCancel()
    }
  }

  const handleZoom = () => {
    if (onChartExpand && data) {
      // 확대 모드로 전환
      const title = stripHtmlTags(currentTitle) || '차트'
      onChartExpand(data, title)
    } else if (window.Plotly && chartRef.current) {
      // 일반 확대/축소 초기화
      window.Plotly.relayout(chartRef.current, {
        'xaxis.autorange': true,
        'yaxis.autorange': true
      })
    }
  }

  const handleDownload = () => {
    if (window.Plotly && chartRef.current) {
      const filename = currentTitle ? currentTitle.replace(/[^a-z0-9]/gi, '_') : 'chart'
      window.Plotly.downloadImage(chartRef.current, {
        format: 'png',
        width: 1200,
        height: 800,
        filename: filename
      })
    }
  }

  const handleColorChange = (traceIndex: number, color: string) => {
    if (window.Plotly && chartRef.current && data) {
      const plotData = data.data || []
      const updatedData = plotData.map((trace: any, idx: number) => {
        if (idx === traceIndex) {
          return {
            ...trace,
            marker: { ...trace.marker, color: color },
            line: trace.line ? { ...trace.line, color: color } : undefined
          }
        }
        return trace
      })

      // 배경색과 그리드 색상을 유지하면서 업데이트
      const updatedLayout = {
        ...data.layout,
        paper_bgcolor: '#ffffff',
        plot_bgcolor: '#ffffff',
        xaxis: {
          ...data.layout?.xaxis,
          gridcolor: '#f3f4f6',
          zerolinecolor: '#e5e7eb',
        },
        yaxis: {
          ...data.layout?.yaxis,
          gridcolor: '#f3f4f6',
          zerolinecolor: '#e5e7eb',
        }
      }

      // data 객체 자체를 업데이트하여 확대 시에도 색상이 유지되도록 함
      data.data = updatedData
      data.layout = updatedLayout

      window.Plotly.react(chartRef.current, updatedData, updatedLayout)

      const newTraceColors = { ...traceColors, [traceIndex]: color }
      setTraceColors(newTraceColors)
      setActiveColorPicker(null)

      // Firestore에 색상 변경사항 저장
      if (messageId && onColorChange) {
        onColorChange(messageId, newTraceColors)
      }
    }
  }

  // 외부 클릭 감지하여 드롭다운 닫기
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      const target = event.target as Node

      // 색상 선택 드롭다운 외부 클릭 확인
      const clickedOutsideColorPicker = colorPickerRef.current && !colorPickerRef.current.contains(target)

      // 색상 팔레트 외부 클릭 확인 (팔레트가 열려있는 경우에만)
      const clickedOutsidePalette = paletteRef.current ? !paletteRef.current.contains(target) : true

      if (clickedOutsideColorPicker && clickedOutsidePalette) {
        setShowColorPicker(false)
        setActiveColorPicker(null)
      }
    }

    if (showColorPicker) {
      document.addEventListener('mousedown', handleClickOutside)
      return () => {
        document.removeEventListener('mousedown', handleClickOutside)
      }
    }
  }, [showColorPicker, activeColorPicker])

  useEffect(() => {
    const loadPlotly = async () => {
      if (typeof window !== 'undefined' && !window.Plotly) {
        try {
          const Plotly = await import('plotly.js-dist-min')
          window.Plotly = Plotly
        } catch (error) {
          console.error('Failed to load Plotly:', error)
          return
        }
      }

      if (window.Plotly && chartRef.current && data) {
        try {
          // Clear any existing chart
          window.Plotly.purge(chartRef.current)
          
          // Improve hover behavior for bar charts
          const plotData = data.data || []
          
          // 개선된 제목 생성
          const generatedTitle = generateBetterTitle(plotData, data.layout)
          if (!currentTitle) {
            setCurrentTitle(generatedTitle)
          }
          
          const layoutConfig = {
            ...data.layout,
            title: {
              text: currentTitle || generatedTitle,
              font: { size: 16, color: '#1f2937' },
              x: 0.02,
              xanchor: 'left'
            },
            hovermode: 'x unified', // Show tooltip for all traces at the same x position
            hoverlabel: {
              bgcolor: 'rgba(0,0,0,0.8)',
              bordercolor: 'rgba(0,0,0,0.8)',
              font: { color: 'white', size: 12 }
            },
            // 배경색 설정
            paper_bgcolor: '#ffffff',  // 전체 배경색 흰색
            plot_bgcolor: '#ffffff',   // 차트 영역 배경색 흰색
            // Disable zoom and pan in axes
            xaxis: {
              ...data.layout?.xaxis,
              fixedrange: true,  // Prevent zooming on x-axis
              gridcolor: '#f3f4f6',  // 아주 옅은 회색 그리드 선
              zerolinecolor: '#e5e7eb',  // 0선 색상
            },
            yaxis: {
              ...data.layout?.yaxis,
              fixedrange: true,  // Prevent zooming on y-axis
              gridcolor: '#f3f4f6',  // 아주 옅은 회색 그리드 선
              zerolinecolor: '#e5e7eb',  // 0선 색상
            },
            // Disable dragmode
            dragmode: false,
          }
          
          // Enhance hover behavior for bar charts specifically and apply saved colors
          const enhancedData = plotData.map((trace, index) => {
            const baseTrace = {
              ...trace,
              hoverlabel: trace.type === 'bar' ? {
                bgcolor: 'rgba(0,0,0,0.8)',
                bordercolor: 'rgba(0,0,0,0.8)',
                font: { color: 'white' }
              } : undefined
            }

            // 저장된 색상이 있으면 적용
            if (savedColors && savedColors[index]) {
              return {
                ...baseTrace,
                marker: { ...baseTrace.marker, color: savedColors[index] },
                line: baseTrace.line ? { ...baseTrace.line, color: savedColors[index] } : undefined
              }
            }

            return baseTrace
          })

          // 저장된 색상을 traceColors 상태에 적용
          if (savedColors) {
            setTraceColors(savedColors)
          }

          // Create new plot
          await window.Plotly.newPlot(
            chartRef.current,
            enhancedData,
            layoutConfig,
            {
              responsive: true,
              displayModeBar: false,  // Hide default toolbar (we have our custom buttons)
              scrollZoom: false,      // Disable zoom with mouse wheel
              doubleClick: false,     // Disable double click actions
              showTips: false,        // Disable hover tooltips on axes
              staticPlot: false,      // Keep interactivity but limit it
              dragMode: false,        // Disable drag to zoom/pan
              // Disable all zoom/pan interactions
              modeBarButtonsToRemove: ['zoom2d', 'pan2d', 'select2d', 'lasso2d', 'zoomIn2d', 'zoomOut2d', 'autoScale2d', 'resetScale2d'],
              // Keep only hover
              editable: false
            }
          )

          // 제목에 호버 및 클릭 이벤트 추가
          if (showTitleEditor && chartRef.current) {
            const titleElement = chartRef.current.querySelector('.gtitle')
            if (titleElement) {
              // 호버 효과를 위한 래퍼 추가
              const titleText = titleElement.querySelector('text')
              if (titleText) {
                titleText.style.cursor = 'pointer'

                // 호버 이벤트
                titleText.addEventListener('mouseenter', () => {
                  setTitleHovered(true)
                })
                titleText.addEventListener('mouseleave', () => {
                  setTitleHovered(false)
                })

                // 클릭 이벤트
                titleText.addEventListener('click', (e) => {
                  e.stopPropagation()
                  handleTitleEdit()
                })
              }
            }
          }
        } catch (error) {
          console.error('Error rendering chart:', error)
        }
      }
    }

    loadPlotly()
  }, [data, showTitleEditor])

  if (!data) {
    return (
      <div 
        className="flex items-center justify-center bg-gray-50 rounded-lg border-2 border-dashed border-gray-300"
        style={{ height }}
      >
        <div className="text-center text-gray-500">
          <svg className="w-12 h-12 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
          </svg>
          <p>차트를 생성할 수 없습니다</p>
        </div>
      </div>
    )
  }

  return (
    <div className="bg-white rounded-lg border border-gray-200 relative" style={{ overflow: 'visible' }}>
      <div
        ref={chartRef}
        style={{ height, width: '100%', overflow: 'hidden' }}
        className="plotly-chart-container rounded-lg"
      />

      {/* 제목 호버 시 편집 아이콘 표시 */}
      {showTitleEditor && titleHovered && (
        <div className="absolute top-4 left-20 pointer-events-none z-10">
          <FiEdit2 className="w-4 h-4 text-gray-500" />
        </div>
      )}

      {/* 제목 편집 모달 */}
      {showTitleModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[200]">
          <div
            ref={titleModalRef}
            className="bg-white rounded-lg shadow-xl p-6 w-96 max-w-[90%]"
          >
            <h3 className="text-lg font-semibold text-gray-800 mb-4">차트 제목 수정</h3>
            <input
              type="text"
              value={editTitle}
              onChange={(e) => setEditTitle(e.target.value)}
              onKeyDown={handleKeyPress}
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent"
              autoFocus
              placeholder="차트 제목을 입력하세요"
            />
            <div className="flex justify-end gap-2 mt-4">
              <button
                onClick={handleTitleCancel}
                className="px-4 py-2 text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200 transition-colors"
              >
                취소
              </button>
              <button
                onClick={handleTitleSave}
                className="px-4 py-2 text-white bg-orange-500 rounded-md hover:bg-orange-600 transition-colors"
              >
                확인
              </button>
            </div>
          </div>
        </div>
      )}

      {/* Floating Chart Controls - 조건부 렌더링 */}
      {data && showControls && (
        <div className="absolute top-16 right-4 flex gap-2 z-50">
          <div className="relative z-50">
            <button
              onClick={() => setShowColorPicker(!showColorPicker)}
              className="p-1 hover:bg-gray-100 rounded transition-colors duration-200 relative z-50"
              title="색상 커스텀"
            >
              <FiSliders className="w-4 h-4 text-gray-600 hover:text-gray-800" />
            </button>

            {/* Color Picker Dropdown */}
            {showColorPicker && (
              <div ref={colorPickerRef} className="absolute top-full left-0 mt-2 w-64 bg-white rounded-lg shadow-2xl p-4 z-[100] border border-gray-200">
                <div className="text-gray-800 text-sm font-medium mb-3">색상</div>
                <div className="space-y-3">
                  {(data.data || []).map((trace: any, index: number) => {
                    const traceName = trace.name || `Series ${index + 1}`
                    // data.data에서 최신 색상을 가져옴
                    const actualTrace = data.data[index]
                    const currentColor = traceColors[index] || actualTrace?.marker?.color || actualTrace?.line?.color || '#f97316'

                    return (
                      <div key={index} className="flex items-center justify-between hover:bg-orange-50 p-2 rounded-lg transition-colors">
                        <span className="text-gray-800 text-sm flex-1 truncate font-medium">{traceName}</span>
                        <div className="relative">
                          <button
                            onClick={() => setActiveColorPicker(activeColorPicker === index ? null : index)}
                            className="w-14 h-7 rounded-md border-2 border-orange-300 hover:border-orange-500 transition-colors shadow-sm"
                            style={{ backgroundColor: currentColor }}
                          />
                          {activeColorPicker === index && (
                            <div ref={paletteRef} className="absolute top-full left-0 mt-2 p-3 bg-white rounded-lg shadow-2xl z-[110] border border-gray-200 w-64">
                              <div className="grid grid-cols-5 gap-2">
                                {['#ef4444', '#f97316', '#f59e0b', '#eab308', '#84cc16',
                                  '#22c55e', '#10b981', '#14b8a6', '#06b6d4', '#0ea5e9',
                                  '#3b82f6', '#6366f1', '#8b5cf6', '#a855f7', '#d946ef',
                                  '#ec4899', '#f43f5e', '#64748b', '#475569', '#1e293b'].map(color => (
                                  <button
                                    key={color}
                                    onClick={() => handleColorChange(index, color)}
                                    className="w-10 h-10 rounded-md hover:scale-110 transition-transform border-2 border-gray-200 hover:border-orange-400 shadow-sm"
                                    style={{ backgroundColor: color }}
                                  />
                                ))}
                              </div>
                            </div>
                          )}
                        </div>
                      </div>
                    )
                  })}
                </div>
              </div>
            )}
          </div>

          <button
            onClick={handleZoom}
            className="p-1 hover:bg-gray-100 rounded transition-colors duration-200"
            title="차트 확대/축소 초기화"
          >
            <FiMaximize2 className="w-4 h-4 text-gray-600 hover:text-gray-800" />
          </button>
          <button
            onClick={handleDownload}
            className="p-1 hover:bg-gray-100 rounded transition-colors duration-200"
            title="차트 다운로드 (PNG)"
          >
            <FiDownload className="w-4 h-4 text-gray-600 hover:text-gray-800" />
          </button>
        </div>
      )}
    </div>
  )
}