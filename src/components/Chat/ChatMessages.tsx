  ...
  return paragraphs.map((paragraph, pIndex) => {
    ...
    while (i < lines.length) {
      ...
      if (numberListMatch) {
        elements.push(
          <div key={i} className="flex items-start mb-1">
            <span className="mr-2 text-orange-500 font-semibold">{numberListMatch[1]}.</span>
...
export default function ChatMessages({ messages, isLoading, messagesEndRef, onFollowUpClick, onChartExpand, uploadedFile, user, onMessageUpdate }: ChatMessagesProps) {
  ...
  const handleChartColorChange = async (messageId: string, colors: {[key: number]: string}) => {
    if (onMessageUpdate) {
      onMessageUpdate(messageId, { chartColors: colors })
    }
  }
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 space-y-8 h-full text-black">
          ...
          <div className={`max-w-[90%] ${message.type === 'user' ? 'order-2' : 'order-1'}`}>
            ...
            <div className={`flex items-center mb-2 ${message.type === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`flex items-center justify-center text-sm font-semibold ${
                message.type === 'user'
                  ? 'w-8 h-8 rounded-full text-black'
                  : ''
              }`} style={message.type === 'user' ? { backgroundColor: '#fed7aa' } : {}}>
              ...
              {message.type === 'user' && (
                <span className="ml-2 text-xs mr-2 order-1 text-gray-600">
            ...
            {message.type === 'user' ? (
              <div>
                <div className="rounded-2xl p-4 shadow-sm text-black" style={{ backgroundColor: '#fed7aa' }}>
              ...
              <div className="text-black">
                ...
                {/* 2. Chart - 코드박스 7px 아래에 표시 (로딩 중이거나 차트 데이터가 있을 때) */}
                {message.codeExecution && (
                  <div style={{ marginTop: '7px' }}>
                    {message.chartData ? (
                      <PlotlyChart
                        ...
                        savedColors={message.chartColors}
                      />
                    ) : message.codeExecution.isExecuting ? (
                      <div className="flex items-center justify-center p-8 bg-gray-50 rounded-lg border border-gray-200">
                        <div className="flex flex-col items-center gap-3">
                          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-orange-500"></div>
                          <span className="text-sm text-gray-600">그래프 생성 중...</span>
                        </div>
                      </div>
                    ) : null}
                  </div>
                ...
                {/* 3. Answer Text - 차트 7px 아래에 표시 */}
                <div style={{ marginTop: message.codeExecution ? '7px' : '0' }}>
                  <TypewriterMessage
