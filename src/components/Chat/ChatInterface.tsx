...
export default function ChatInterface() {
  ...
  const handleMessageUpdate = async (messageId: string, updates: Partial<Message>) => {
    ...
    // Firestore 업데이트 (chartColors가 변경된 경우)
    if (updates.chartColors && currentSessionId) {
      try {
        const { getDoc, setDoc, doc } = await import('firebase/firestore')
        const { db } = await import('@/lib/firebase')

        // chat 문서에서 메시지 배열 가져오기
        const chatRef = doc(db, 'chat', currentSessionId)
        ...
        if (chatSnap.exists()) {
          ...
          // 해당 메시지 찾아서 chartColors 업데이트
          const updatedMessages = messages.map((msg: any) =>
            msg.id === messageId ? { ...msg, chartColors: updates.chartColors } : msg
          )
          ...
          console.log('✅ Chart colors saved to Firestore')
        }
      } catch (error) {
        console.error('차트 색상 저장 실패:', error)
      }
    }
  }
          ...
          await apiService.askUnifiedQuestionStream(
            ...
            (() => {
              ...
              return (chunk) => {
                      ...
                      switch (chunk.type) {
                        ...
                        case 'text_stream':
                          ...
                          if (canAddText && chunk.content) {
                            ...
                            // 첫 번째 text_stream이면 기존 로딩 메시지를 지우고 새로 시작
                            const isLoadingMessage = updatedMessage.content === '처리 중...' ||
                                                    updatedMessage.content === '분석을 시작합니다...' ||
                                                    updatedMessage.content === '질문을 분석하고 있습니다...' ||
                                                    updatedMessage.content === '계산 중입니다...' ||
                                                    updatedMessage.content === '코드를 생성 중입니다...' ||
                                                    updatedMessage.content === '실행 중입니다...'
                        ...
                        case 'stream_done':
                          // 스트리밍 완료 신호 - 아무것도 하지 않음 (onComplete에서 처리)
                          break
                ...
                // 메시지 업데이트 후 자동 스크롤 (사용자가 수동으로 스크롤하지 않았을 때만)
    ...
    <div className="flex h-screen w-full max-w-none mx-auto overflow-hidden">
      ...
      >
        ...
        <div className={`p-4 ${
          ...
          {isSidebarCollapsed ? (
            <button
              ...
              className="p-1 hover:bg-orange-100 rounded transition-colors relative group"
            >
              <Image
              ...
              />
              <RiSideBarLine className="w-4 h-4 absolute inset-0 m-auto opacity-0 group-hover:opacity-100 transition-opacity duration-200 text-black" />
            </button>
          ) : (
            <>
              <div className="flex items-center space-x-2">
                ...
                />
                <span className="font-bold text-black" style={{fontFamily: 'SpectralLight, serif'}}>Afterwon</span>
              </div>
              <button
                onClick={(e) => {
                  e.preventDefault()
              ...
              >
                <RiSideBarLine className="w-4 h-4 text-black" />
              </button>
            </>
          )}
        </div>
        ...
        <div className="p-3 space-y-1">
          ...
          <button 
            ...
            className={`w-full flex items-center rounded-lg hover:bg-orange-100 transition-colors ${
              isSidebarCollapsed ? 'justify-center p-2' : 'space-x-3 px-3 py-2 text-left'
            } ${currentPage === 'chatsearch' ? 'bg-orange-100' : ''}`}
          >
            <svg className="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button
            ...
            className={`w-full flex items-center rounded-lg hover:bg-orange-100 transition-colors ${
              isSidebarCollapsed ? 'justify-center p-2' : 'space-x-3 px-3 py-2 text-left'
            } ${currentPage === 'projects' ? 'bg-orange-100' : ''}`}
          >
            <svg className="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button 
            ...
            className={`w-full flex items-center rounded-lg hover:bg-orange-100 transition-colors ${
              isSidebarCollapsed ? 'justify-center p-2' : 'space-x-3 px-3 py-2 text-left'
            } ${currentPage === 'filesystem' ? 'bg-orange-100' : ''}`}
          >
            <svg className="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {isSidebarCollapsed ? (
          ...
          <>
            ...
            <div className="px-3 py-2" style={{ marginTop: '24px' }}>
              <h3 className="text-xs font-medium text-black uppercase tracking-wider mb-2">
                {t('nav.recentItems')}
              </h3>
            </div>
                ...
                chatHistory.map((session) => (
                  <div
                    ...
                    className={`relative group w-full rounded-lg transition-colors ${
                      currentSessionId === session.id ? 'bg-orange-100' : 'hover:bg-orange-100'
                    }`}
                  >
                    <button
                      onClick={(e) => {
                    ...
                    <button
                      ...
                      className="absolute right-1 top-1/2 transform -translate-y-1/2 p-1 rounded hover:bg-orange-200 opacity-0 group-hover:opacity-100 transition-opacity"
                    >
                    ...
                    {openMenuId === session.id && (
                      <div data-menu-container className="absolute right-0 top-8 mt-1 w-48 bg-white rounded-md shadow-lg border border-gray-300 z-50">
                        <div className="py-1">
                          <button
                          ...
                          <button
                            ...
                            className="flex items-center w-full px-4 py-2 text-sm text-red-600 hover:bg-orange-50"
                          >
                            <svg className="w-4 h-4 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="p-3 relative" data-profile-menu>
          {isSidebarCollapsed ? (
            ...
            <button
              ...
              className={`w-full flex items-center rounded-lg hover:bg-orange-100 transition-colors justify-center p-2`}
            >
            ...
            <button
              onClick={() => setIsProfileMenuOpen(!isProfileMenuOpen)}
              className="w-full flex items-center space-x-3 p-2 rounded-lg hover:bg-orange-100 transition-colors"
            >
          ...
          {isProfileMenuOpen && !isSidebarCollapsed && (
            <div className="absolute bottom-full left-3 right-3 mb-2 bg-white rounded-lg border border-gray-300 shadow-xl z-50">
              {/* User Email Header */}
              ...
              <div className="py-2">
                ...
                <button className="w-full px-4 py-2 text-left text-sm text-gray-700 hover:bg-orange-50 transition-colors">
                  {t('profile.help')}
                </button>
              </div>
              ...
              <div className="border-t border-gray-200 py-2">
                ...
                <button className="w-full px-4 py-2 text-left text-sm text-gray-700 hover:bg-orange-50 transition-colors flex items-center justify-between">
      ...
      {expandedChart ? (
        ...
        <div className="flex flex-1 h-screen overflow-hidden min-w-0">
          {/* 좌측 채팅 패널 */}
          <div
            className="flex flex-col bg-white overflow-hidden min-w-0"
          ...
          >
            ...
            <div className="p-4 bg-white flex-shrink-0 relative z-10">
          ...
          <div
            ref={resizerRef}
            className="w-1 bg-orange-500 hover:bg-orange-600 cursor-col-resize flex-shrink-0 relative group"
          >
            <div className="absolute inset-y-0 -inset-x-1 group-hover:bg-orange-500 transition-colors opacity-20"></div>
          </div>
        ...
        // 일반 모드: 기존 레이아웃
        <div className="flex flex-col bg-white flex-1 h-screen min-w-0 overflow-hidden">
          {currentPage === 'chat' ? (
            <>
              <ChatHeader uploadedFile={uploadedFile} />
