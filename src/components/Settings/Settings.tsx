...
export default function Settings({ user }: SettingsProps) {
  ...
  const renderProfileSection = () => (
    <div className="space-y-8">
      {/* 성명 섹션 */}
      <div>
        <label className="block text-black text-sm font-medium mb-4">{t('settings.name')}</label>
        <div className="flex items-center space-x-4">
          <div className="w-12 h-12 bg-orange-200 rounded-full flex items-center justify-center">
            <span className="text-black font-medium">{getUserInitial(user)}</span>
          </div>
          <input
            type="text"
            value={getUserDisplayName(user)}
            className="flex-1 bg-white border border-gray-300 rounded-lg px-4 py-3 text-black focus:outline-none focus:border-orange-400"
            readOnly
          />
        </div>
      </div>
      ...
      <div>
        <label className="block text-black text-sm font-medium mb-4">
        ...
        <div className="relative" ref={dropdownRef}>
          <button
            ...
            className={`w-full bg-white border border-gray-300 rounded-lg px-4 py-3 text-black focus:outline-none focus:border-orange-400 text-left flex items-center justify-between ${isLoadingJob ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
            <span className={selectedJob === t('settings.jobPlaceholder') ? 'text-gray-400' : 'text-black'}>
            ...
            <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-300 rounded-lg shadow-lg z-50 max-h-60 overflow-y-auto">
              {jobOptions.map((option) => (
                <button
                  ...
                  disabled={isLoadingJob}
                  className={`w-full px-4 py-3 text-left text-black transition-colors first:rounded-t-lg last:rounded-b-lg ${isLoadingJob ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-100'}`}
                >
                  {option.label}
                </button>
              ))}
      ...
      <div>
        ...
        <textarea
          ...
          className="w-full bg-white border border-gray-300 rounded-lg px-4 py-3 text-black placeholder-gray-400 focus:outline-none focus:border-orange-400 resize-none"
          rows={4}
        />
      </div>
  ...
  const renderLanguageSection = () => (
    <div className="space-y-8">
      <div>
        <h2 className="text-xl font-semibold text-black mb-6">{t('settings.language')}</h2>
        <div>
          <label className="block text-black text-sm font-medium mb-4">
          ...
          <div className="relative" ref={interfaceLanguageRef}>
            <button
              ...
              className="w-full bg-white border border-gray-300 rounded-lg px-4 py-3 text-black focus:outline-none focus:border-orange-400 text-left flex items-center justify-between"
            >
                ...
                {languageOptions.map((option) => (
                  <button
                    ...
                    className={`w-full px-4 py-3 text-left text-black hover:bg-gray-100 transition-colors first:rounded-t-lg last:rounded-b-lg ${
                      language === option.code ? 'bg-orange-100' : ''
                    }`}
                  >
                    {option.label}
                  </button>
  ...
  const renderContent = () => {
    switch (activeSection) {
      ...
      case 'account':
        return <div className="text-black">{t('settings.accountContent')}</div>
      case 'privacy':
        return <div className="text-black">{t('settings.privacyContent')}</div>
      case 'billing':
        return <div className="text-black">{t('settings.billingContent')}</div>
      case 'features':
        return <div className="text-black">{t('settings.featuresContent')}</div>
      default:
        return <div className="text-black">{t('settings.selectContent')}</div>
    }
  }
  ...
  return (
    <div className="flex h-full bg-white text-black">
      ...
      <div className="w-64 bg-gray-50 border-r border-gray-200">
        <div className="p-6">
          <h1 className="text-2xl font-bold text-black mb-8">{t('settings.title')}</h1>
